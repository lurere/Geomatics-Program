...
namespace 电离层延迟改正
{
    class Algorithm
    {
        ...
        //{
            ...
            //return (deg + min / 60.0 + sec / 3600.0) * Math.PI / 180.0;//弧度转角度的公式
        ...
        /// <summary>
        /// 判断一个角度 a 在标准的 0 到 2π 之间的值，并且根据输入的 sa1 和 t 来调整角度的取值范围
        /// </summary>
        /// <param name="sa1"></param>
        /// <param name="t"></param>
        /// <param name="a"></param>
        /// <returns></returns>
        public double JudgeAngle(double sa1, double t, double a)//保证在0-360之间
        {
            if (sa1 > 0 && t > 0)
            {
                a = Math.Abs(a);//绝对值
        ...
        public void CalAE(DataCenter dataCenter)//无返回值吗
        {
            double sbp = Math.Sin(dataCenter.Bp);//计算测站纬度的正弦值，并将结果赋值给变量 sbp
            double cbp = Math.Cos(dataCenter.Bp);
            double slp = Math.Sin(dataCenter.Lp);//计算测站经度的正弦值，并将结果赋值给变量 slp
            ...
            foreach (var sat in dataCenter.satellites)
            {
                ...
                dx = sat.X - dataCenter.Station_x;//卫星 X 坐标与测站 X 坐标的差值
                dy = sat.Y - dataCenter.Station_y;
                dz = sat.Z - dataCenter.Station_z;

                x = -sbp * clp * dx - sbp * slp * dy + cbp * dz;//卫星与测站的坐标差值进行坐标转换，并将转换后的 X 坐标赋值给变量 x。
                y = -slp * dx + clp * dy;
        ...
        /// </summary>
        /// <param name="dataCenter"></param>
        public void CalIonDely(DataCenter dataCenter)//接受一个 DataCenter 类型的参数 dataCenter，用来计算每个卫星的电离层延迟。
        {
           foreach (var sat in dataCenter.satellites)
            {
                ...
                double svE = sat.E / Math.PI;//纬度转换为弧度制
                ...
                if (t < 0) t += 86400;
                if (t > 86400) t -= 86400;//将当地时间 t 限制在 0 到 86400 秒之间

                // 计算电离层延迟改正
                double F, judge, Tion, c = 299792458.0;
                ...
                if (judge < 1.57) Tion = F * (A1 + A2 * (1 - judge * judge / 2 + judge * judge * judge * judge / 24));
                else Tion = F * A1;
